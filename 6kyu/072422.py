...
#
# M = ["A", "B", "C", "W"] or ...
# and your task is to find all the books of L with codes belonging to each category of M and to
...
def stock_list(listOfArt, listOfCat):
    ...
    items_in_stock = {}
    for info, category in enumerate(listOfCat):
        for art in all_items:
            if art[0].startswith(category):
                if category in items_in_stock:
                    items_in_stock[category] += int(art[1])
                else:
                    items_in_stock[category] = int(art[1])
            else:
                if category not in items_in_stock:
                    items_in_stock[category] = 0
    answer = '('
