...
def find_admin(lst, lang):
    language_lst = []
    for entry in lst:
        if entry['language'] == lang and entry['githubAdmin'] == 'yes':
            language_lst.append(entry)
    return language_lst
...
find_admin(list1, 'JavaScript') # => answer1
find_admin(list1, 'Ruby') # => []
find_admin(list1, 'Python') # => []
